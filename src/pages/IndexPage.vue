		...
		<div class="bannerClss">
			<van-swipe class="card-swiper" :class="dotStyle?'square-dot':'round-dot'" :indicator-dots="true" :circular="true"
			 ...
			 indicator-active-color="#fa553c">
				<van-swipe-item v-for="(item,index) in swiperList" :key="index" :class="cardCur==index?'cur':''">
					<div class="swiper-item" :style="'transform: scale(calc'+(0.5 +index) / 10+')); margin-left: calc'+( item.mLeft * 50 - 75)+'px;z-index:'+index">
						<img :src="fmnt+item.image" mode="aspectFill"></img>
					</div>
				</van-swipe-item>
			</van-swipe>
		</div>
		...
		<div class="noticeClass">
			...
			<div class="noticeRight">
				<van-notice-bar :scrollable="true">
					<van-swipe vertical class="notice-swipe" :autoplay="3000" :show-indicators="false" v-for="(item,index) in noticList"
					 :key="index">
						<van-swipe-item>
							<div class="noticeLine">
								<label>[红球]</label>
								<span>{{item.depict}}</span>
							</div>
						</van-swipe-item>
					</van-swipe>
				</van-notice-bar>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getIndexData() {
				var that = this;
				this.axios.post('soccerSlideshow/getPageList.do?',{
					mubanId:'10015',
					pageIndex:1,
					pageSize:50
				}).then((res)=>{
					...
					console.log(res,'---首页轮播')
				})
				//查询消息通知栏
				this.axios.post('SoccerNotification/getPageList.do?',{
					mubanId:'10015',
					pageIndex:1,
					pageSize:3
				}).then((res)=>{
					if(res.data.code == 200){
						that.noticList = res.data.data;
					}else{
						that.noticList = that.swiperList;
					}
					console.log(res,'---首页消息通知栏')
				})
				//查询人工智能数据
				this.axios.post('soccerSmart/getPageList.do?',{
					mubanId:'10015',
					pageIndex:1,
					pageSize:5
				}).then((res)=>{
					if(res.data.code == 200){
						...
						that.dataList = that.swiperList;
					}
					console.log(res,'---首页人工智能')
				})
			},
			// cardSwiper
			cardSwiper(e) {
